...
struct Ld2410HighResolutionSample {
  ...
  // v2.4.6 engineering-mode evidence.
  bool engineeringData;
  uint8_t maxMovingGate;
  uint8_t maxStationaryGate;
  uint8_t lightValue;
  uint8_t outPinState;
  uint8_t movingGateEnergy[9];
  uint8_t stationaryGateEnergy[9];
};
...
// ============================================================
// v2.4.7 DEVELOPMENT-ONLY ADAPTIVE HIGH-RES CAPTURE CONTROLLER v1
//
// Safety primitive for server-directed engineering-data capture.
//
// The server may request a bounded capture window, but the ESP32 owns
// the deadline. Once started, capture will terminate locally even if
// Wi-Fi, MQTT, or the server becomes unavailable.
//
// This controller does NOT change GPIO21 occupancy authority or make
// activity, fall, emergency, medical, or care classifications.
// ============================================================

static constexpr unsigned long
  LD2410_HIGH_RES_CAPTURE_DEFAULT_MS = 30000UL;

static constexpr unsigned long
  LD2410_HIGH_RES_CAPTURE_MIN_MS = 5000UL;

static constexpr unsigned long
  LD2410_HIGH_RES_CAPTURE_MAX_MS = 120000UL;

bool ld2410TimedCaptureActive = false;
unsigned long ld2410TimedCaptureStartedAt = 0;
unsigned long ld2410TimedCaptureDurationMs = 0;
String ld2410TimedCaptureCommandId;
String ld2410TimedCaptureMode;
String ld2410TimedCaptureReason;
...
// v2.4.6 development-only engineering telemetry.
// These values never become occupancy authority.
bool ld2410EngineeringData = false;
uint8_t ld2410MaxMovingGate = 0;
uint8_t ld2410MaxStationaryGate = 0;
uint8_t ld2410LightValue = 0;
uint8_t ld2410OutPinState = 0;
uint8_t ld2410MovingGateEnergy[9] = {0};
uint8_t ld2410StationaryGateEnergy[9] = {0};
...
static constexpr uint16_t MQTT_BUFFER_SIZE = 4096;
...
unsigned long extractJsonUnsignedLong(
  ...
  while (i < (int)json.length()) {
    ...
    if (c >= '0' && c <= '9') {
      ...
      if (value > (4294967295UL - digit) / 10UL) {
        return fallback;
      }
...
// ============================================================
// v2.4.6 DEVELOPMENT-ONLY LD2410 ENGINEERING COMMAND SUPPORT
//
// Command frame:
//   FD FC FB FA
//   length LE
...
//   04 03 02 01
//
// Engineering commands are observer-only development controls.
// They do not change GPIO21 occupancy authority.
// ============================================================

void sendLd2410Command(
  uint16_t command,
  ...
  const uint16_t payloadLength =
    (uint16_t)(2U + dataLength);

  LD2410Serial.write(header, sizeof(header));
  ...
  if (data != nullptr && dataLength > 0) {
    LD2410Serial.write(data, dataLength);
  }

  LD2410Serial.write(footer, sizeof(footer));
  LD2410Serial.flush();
}

void enterLd2410ConfigurationMode() {
  const uint8_t enableData[2] = {
    0x01, 0x00
  };
...
void exitLd2410ConfigurationMode() {
  sendLd2410Command(0x00FE);
  delay(100);
}

void enableLd2410EngineeringMode() {
  enterLd2410ConfigurationMode();
  sendLd2410Command(0x0062);
  delay(100);
  exitLd2410ConfigurationMode();
}

void disableLd2410EngineeringMode() {
  enterLd2410ConfigurationMode();
  sendLd2410Command(0x0063);
  delay(100);
  exitLd2410ConfigurationMode();
}

void clearLd2410TimedCaptureState() {
  ld2410TimedCaptureActive = false;
  ld2410TimedCaptureStartedAt = 0;
  ld2410TimedCaptureDurationMs = 0;
  ld2410TimedCaptureCommandId = "";
  ld2410TimedCaptureMode = "";
  ld2410TimedCaptureReason = "";
}

void stopLd2410HighResolutionCapture(
  const char* stopReason
) {
  // Stop evidence collection before changing LD2410 mode so no
  // transition frame can later escape as high-resolution evidence.
  ld2410HighResolutionEnabled = false;
  ld2410HighResolutionBatchCount = 0;
  ld2410HighResolutionBatchStartedAt = 0;

  disableLd2410EngineeringMode();
  clearLd2410TimedCaptureState();

  logLine(
    "High-resolution capture stopped reason=" +
    String(stopReason ? stopReason : "unspecified")
  );
}

bool startLd2410TimedCapture(
  const String& commandId,
  ...
  unsigned long durationMs = requestedDurationMs;

  if (durationMs == 0) {
    durationMs = LD2410_HIGH_RES_CAPTURE_DEFAULT_MS;
  }

  if (durationMs < LD2410_HIGH_RES_CAPTURE_MIN_MS) {
    durationMs = LD2410_HIGH_RES_CAPTURE_MIN_MS;
  }

  if (durationMs > LD2410_HIGH_RES_CAPTURE_MAX_MS) {
    durationMs = LD2410_HIGH_RES_CAPTURE_MAX_MS;
  }

  // Clear stale evidence before entering engineering mode.
  ld2410HighResolutionEnabled = false;
  ld2410HighResolutionBatchCount = 0;
  ld2410HighResolutionBatchStartedAt = 0;

  enableLd2410EngineeringMode();

  ld2410HighResolutionEnabled = true;
  ld2410TimedCaptureActive = true;
  ld2410TimedCaptureStartedAt = millis();
  ld2410TimedCaptureDurationMs = durationMs;
  ld2410TimedCaptureCommandId = commandId;
  ld2410TimedCaptureMode = mode;
  ld2410TimedCaptureReason = reason;

  logLine(
    "Timed high-resolution capture started durationMs=" +
    String(durationMs) +
    " mode=" + mode +
    " reason=" + reason
  );
...
void serviceLd2410TimedCaptureSafety() {
  if (!ld2410TimedCaptureActive) {
    return;
  }

  // Unsigned subtraction intentionally provides millis() rollover safety.
  unsigned long elapsed =
    millis() - ld2410TimedCaptureStartedAt;

  if (elapsed < ld2410TimedCaptureDurationMs) {
    return;
  }

  stopLd2410HighResolutionCapture("local_timeout");
}
...
void parseLd2410Frame(const uint8_t* frame, int length) {
  ...
  // LD2410 periodic report:
  //   0x01 = engineering mode
  //   0x02 = normal/basic mode
  //
  // The 0xAA report header remains common to both.
  const bool engineeringFrame =
    frame[6] == 0x01;

  const bool normalFrame =
    frame[6] == 0x02;

  if (
    (!engineeringFrame && !normalFrame) ||
    frame[7] != 0xAA
  ) {
    return;
  }

  // Engineering frames contain the extended gate-energy block.
  // OUT pin is at full-frame byte 38, followed by report tail/check
  // and the four-byte data-frame footer.
  if (engineeringFrame && length < 45) {
    return;
  }
  ...
  if (engineeringFrame) {
    // In engineering mode bytes 17 and 18 are the maximum
    // moving/still distance gates, so detection distance comes
    // from the common basic-data fields at bytes 15 and 16.
    ld2410DetectionDistanceCm =
      ld2410ReadUInt16LE(frame[15], frame[16]);

    ld2410EngineeringData = true;

    ld2410MaxMovingGate = frame[17];
    ld2410MaxStationaryGate = frame[18];

    for (uint8_t gate = 0; gate < 9; ++gate) {
      ld2410MovingGateEnergy[gate] =
        frame[19 + gate];

      ld2410StationaryGateEnergy[gate] =
        frame[28 + gate];
    }

    ld2410LightValue = frame[37];
    ld2410OutPinState = frame[38];

  } else {
    // Preserve the currently validated v2.4.5 normal-mode
    // interpretation. Engineering development must not silently
    // change production behavior.
    ld2410DetectionDistanceCm =
      ld2410ReadUInt16LE(frame[17], frame[18]);

    ld2410EngineeringData = false;
    ld2410MaxMovingGate = 0;
    ld2410MaxStationaryGate = 0;
    ld2410LightValue = 0;
    ld2410OutPinState = 0;

    for (uint8_t gate = 0; gate < 9; ++gate) {
      ld2410MovingGateEnergy[gate] = 0;
      ld2410StationaryGateEnergy[gate] = 0;
    }
  }
  ...
  // Observer-only high-resolution evidence capture.
  if (
    ...
    sample.engineeringData =
      ld2410EngineeringData;

    sample.maxMovingGate =
      ld2410MaxMovingGate;

    sample.maxStationaryGate =
      ld2410MaxStationaryGate;
    ...
    for (uint8_t gate = 0; gate < 9; ++gate) {
      sample.movingGateEnergy[gate] =
        ld2410MovingGateEnergy[gate];

      sample.stationaryGateEnergy[gate] =
        ld2410StationaryGateEnergy[gate];
    }
    ...
    // Never block operational sensor behavior.
...
bool publishLd2410HighResolutionBatch() {
  ...
  // v2.4.6 compact engineering observer transport.
  //
  // compact-array-v2 order:
  ...
  // engineeringData, maxMovingGate, maxStationaryGate,
  // lightValue, outPinState,
  // movingGateEnergy[0...8],
  // stationaryGateEnergy[0...8].
  //
  // Acquisition remains per parsed LD2410 frame.
  p += "\"sampleEncoding\":\"compact-array-v2\",";
  p += "\"samples\":[";
  ...
  for (
    ...
    p += ",";
    p += s.engineeringData ? "1" : "0";
    p += ",";
    p += String(s.maxMovingGate);
    p += ",";
    p += String(s.maxStationaryGate);
    p += ",";
    p += String(s.lightValue);
    p += ",";
    p += String(s.outPinState);

    p += ",[";
    for (uint8_t gate = 0; gate < 9; ++gate) {
      if (gate > 0) p += ",";
      p += String(s.movingGateEnergy[gate]);
    }
    p += "]";
    ...
    for (uint8_t gate = 0; gate < 9; ++gate) {
      if (gate > 0) p += ",";
      p += String(s.stationaryGateEnergy[gate]);
    }
    p += "]";
...
void executeMqttCommand(const String& payload) {
  ...
  if (commandType == "high_res_capture") {
    // Server MQTT v2 places command parameters inside the top-level
    // "payload" object. The existing lightweight JSON helpers search
    // the complete command string, so these keys are resolved from
    // that nested object without changing the validated command envelope.
    unsigned long requestedDurationMs =
      extractJsonUnsignedLong(
        payload,
        "durationMs",
        LD2410_HIGH_RES_CAPTURE_DEFAULT_MS
      );

    String captureMode =
      extractJsonString(payload, "mode");

    String captureReason =
      extractJsonString(payload, "reason");
    ...
    if (captureReason.length() == 0) {
      captureReason = "unspecified";
    }
    ...
    if (ld2410TimedCaptureActive) {
      publishCommandResult(
        ...
        commandType,
        "failed",
        "Timed high-resolution capture already active."
      );
      return;
    }

    bool started =
      startLd2410TimedCapture(
        commandId,
        requestedDurationMs,
        captureMode,
        captureReason
    ...
    if (!started) {
      publishCommandResult(
        ...
        commandType,
        "failed",
        "Timed high-resolution capture could not start."
      );
      return;
    }
    ...
    publishCommandResult(
      ...
      commandType,
      "success",
      "Timed high-resolution engineering capture started."
    );
    return;
  }

  if (commandType == "high_res_enable") {
    // Legacy development command retained for compatibility.
    // Never allow it to convert an active bounded capture into an
    // indefinite engineering-mode session.
    if (ld2410TimedCaptureActive) {
      publishCommandResult(
        commandId,
        commandType,
    ...
    publishCommandResult(
      commandId, commandType, "success",
      "High-resolution engineering evidence enabled."
    );
    return;
  }

  if (commandType == "high_res_disable") {
    // Explicit OFF always wins, including during a timed capture.
    stopLd2410HighResolutionCapture("explicit_disable");

    publishCommandResult(
      commandId, commandType, "success",
      "High-resolution engineering evidence disabled."
    );
    return;
  }
...
void setup() {
  ...
  // Safety invariant: every ESP32 boot explicitly returns the LD2410
  // to normal/basic reporting. Do not rely on an ESP32 reset to
  // power-cycle or reset the radar module.
  delay(250);
  disableLd2410EngineeringMode();
  clearLd2410TimedCaptureState();
  ld2410HighResolutionEnabled = false;
...
void loop() {
  // Local safety authority. This deliberately runs outside every
  // Wi-Fi/MQTT condition so network loss cannot extend a capture.
  serviceLd2410TimedCaptureSafety();
