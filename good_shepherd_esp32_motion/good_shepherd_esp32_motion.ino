...
const char* SOFTWARE_VERSION = "esp32-motion-v1.7.1-ble-safe-ota";
...
String assignmentState() {
  if (locationName.length() == 0 || residentName.length() == 0 || roomName.length() == 0) return "Unassigned";
  return "Assigned";
}
...
void rememberWifiCredential(String ssid, String password) {
  ...
  prefs.begin("gs-device", false);
  int existingIndex = -1, emptyIndex = -1;
...
String buildBleStatusPayload() {
  String wifiStatus = "offline", ip = "", ssid = "";
  int rssi = 0;
  if (WiFi.status() == WL_CONNECTED) { wifiStatus = "connected"; ip = WiFi.localIP().toString(); ssid = WiFi.SSID(); rssi = WiFi.RSSI(); }
  else if (setupModeStarted) wifiStatus = "setup_ap";
  else if (wifiName.length() > 0) wifiStatus = "configured_offline";
  else wifiStatus = "unconfigured";
  ...
  payload += "\"autoInstallOnBoot\":false,";
...
void saveSettings() {
  ...
  prefs.remove("nodeId"); prefs.remove("sourceKey");
...
void clearAssignmentSettingsOnly() {
  prefs.begin("gs-device", false);
  prefs.remove("wifiName"); prefs.remove("wifiPass"); prefs.remove("location"); prefs.remove("resident"); prefs.remove("room"); prefs.remove("deviceName"); prefs.remove("nodeId"); prefs.remove("sourceKey");
  prefs.end();
  wifiName = ""; wifiPassword = ""; locationName = ""; residentName = ""; roomName = ""; deviceName = "Motion Sensor";
  generateHardwareIds();
...
void clearSettingsAndRestart() {
  ...
  clearAssignmentSettingsOnly(); delay(1000); ESP.restart();
}
...
void setup() {
  ...
  setLed(false);
  Serial.println();
  Serial.println("Good Shepherd ESP32 Motion v1.7.1");
  generateHardwareIds();
  ensureSetupId();
  loadSettings();
  startBleManagement();
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (nodeRegistered) handleMotionSensor();
  }
...
void startSetupMode() {
  ...
  setupModeStarted = true; nodeRegistered = false;
...
void stopSetupMode() {
  ...
  server.close(); WiFi.softAPdisconnect(true); setupModeStarted = false;
  delay(250); updateBleStatus();
}
...
void connectToSavedWifi() {
  ...
  WiFi.mode(WIFI_STA); delay(250);
  WiFi.begin(wifiName.c_str(), wifiPassword.c_str());
  int attempts = 0;
  while (WiFi.status() != WL_CONNECTED && attempts < 40) { delay(500); Serial.print("."); attempts++; updateBleStatus(); }
  Serial.println();
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("Wi-Fi failed. Starting setup mode fallback.");
...
void connectToNewWifiFromBle(String ssid, String password, String newLocation, String newResident, String newRoom, String newDeviceName, bool shouldRestartAfterSave) {
  ...
  publishBleResult("running", "Connecting to Wi-Fi.");
  connectToSavedWifi();
  if (WiFi.status() == WL_CONNECTED) publishBleResult("success", "Wi-Fi connected. Firmware status check queued. Auto-install is disabled.");
  else publishBleResult("failed", "Wi-Fi connection failed. Setup hotspot fallback is active.");
}

void handleWifiReconnect() {
  if (wifiName.length() == 0 || setupModeStarted || firmwareUpdateInProgress) return;
  if (WiFi.status() == WL_CONNECTED) return;
  if (millis() - lastReconnectAttempt >= RECONNECT_INTERVAL_MS) {
    lastReconnectAttempt = millis(); nodeRegistered = false;
    Serial.println("Wi-Fi disconnected. Reconnecting...");
    WiFi.disconnect(); WiFi.begin(wifiName.c_str(), wifiPassword.c_str()); updateBleStatus();
  }
}
...
bool registerNode() {
  ...
  HTTPClient http; http.setTimeout(30000); http.setReuse(false);
  http.begin(client, REGISTER_URL);
  http.addHeader("Content-Type", "application/json"); http.addHeader("x-webhook-secret", WEBHOOK_SECRET);
  String payload = "{";
...
bool sendHeartbeat() {
  ...
  payload += "\"monitorStatus\":\"Online\",\"ffmpegStatus\":\"Not Applicable\",\"platform\":\"ESP32\",";
  payload += "\"hostname\":\"" + nodeId + "\",\"activeMonitorCount\":1,";
  payload += "\"diagnostics\":{";
  payload += "\"sourceKey\":\"" + sourceKey + "\",\"setupId\":\"" + setupId + "\",";
  payload += "\"assignmentState\":\"" + assignmentState() + "\",";
  payload += "\"wifiSsid\":\"" + jsonEscape(WiFi.SSID()) + "\",";
  payload += "\"deviceName\":\"" + jsonEscape(deviceName) + "\",";
  payload += "\"roomName\":\"" + jsonEscape(roomName) + "\",";
  ...
  payload += "\"locationName\":\"" + jsonEscape(locationName) + "\",";
  payload += "\"wifiRssi\":" + String(WiFi.RSSI()) + ",";
  payload += "\"bleManagement\":\"Enabled\",\"autoFirmwareInstallOnBoot\":false";
  payload += "}}";
  int responseCode = http.POST(payload);
  Serial.print(sensorLabel() + " | Heartbeat response code: "); Serial.println(responseCode);
  http.end(); client.stop(); delay(50);
  return responseCode >= 200 && responseCode < 300;
}
...
void pollSensorCommands() {
  ...
  WiFiClientSecure client; client.setInsecure();
  HTTPClient http; http.setTimeout(30000); http.setReuse(false);
  http.begin(client, url); http.addHeader("x-webhook-secret", WEBHOOK_SECRET);
  int responseCode = http.GET();
  if (responseCode != 200) { Serial.print(sensorLabel() + " | Command poll response code: "); Serial.println(responseCode); http.end(); client.stop(); delay(50); return; }
  ...
  if (commandType == "update_firmware") {
    ...
    if (firmwareUrl.length() == 0) { reportCommandResult(commandId, "failed", "Firmware update failed: missing firmwareUrl in command payload."); return; }
    performFirmwareUpdate(commandId, firmwareUrl); return;
  }
...
void reportCommandResult(String commandId, String status, String message) {
  ...
  payload += "}}";
  int responseCode = http.POST(payload);
  Serial.print(sensorLabel() + " | Command result response code: "); Serial.println(responseCode);
  http.end(); client.stop(); delay(50);
}
...
bool checkLatestFirmwareStatusOnly() {
  ...
  if (responseCode < 200 || responseCode >= 300) {
    ...
    http.end(); client.stop(); logSensor(message); publishBleResult("failed", message); return false;
  }
  String response = http.getString(); http.end(); client.stop(); delay(50);
...
void performFirmwareUpdate(String commandId, String firmwareUrl) {
  if (WiFi.status() != WL_CONNECTED) {
    ...
    publishBleResult("failed", "Firmware update failed: Wi-Fi offline."); return;
  }
  firmwareUpdateInProgress = true;
  logSensor("Firmware update requested."); logSensor("Firmware URL: " + firmwareUrl);
  WiFiClientSecure client; client.setInsecure();
  HTTPClient http; http.setTimeout(120000); http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS); http.setReuse(false);
  ...
  if (contentLength > 0 && written != (size_t)contentLength) {
    ...
    Update.abort(); http.end(); client.stop(); firmwareUpdateInProgress = false;
    if (commandId.length() > 0) reportCommandResult(commandId, "failed", message);
    publishBleResult("failed", message); return;
  }
  if (!Update.end()) {
  ...
  if (!Update.isFinished()) {
    ...
    publishBleResult("failed", "Firmware update failed: update did not finish."); return;
  }
...
void handleBleCommand(String payload) {
  ...
  if (command.length() == 0) command = extractJsonString(payload, "action");
  command.toLowerCase();
  if (command.length() == 0) { publishBleResult("failed", "Missing BLE command."); return; }
  logSensor("Handling BLE command: " + command);
  if (command == "get_status" || command == "status") { updateBleStatus(); publishBleResult("success", "Status refreshed."); return; }
  ...
  if (command == "force_update" || command == "update_firmware") {
    ...
    if (firmwareUrl.length() == 0) { publishBleResult("failed", "BLE firmware update requires firmwareUrl. Auto-install from latest is disabled."); return; }
    publishBleResult("running", "Firmware update starting from BLE URL."); performFirmwareUpdate("", firmwareUrl); return;
  }
...
void handleMotionSensor() {
  ...
  if (motion == MOTION_ACTIVE_STATE && !motionAlreadyReported) {
    logSensor("Motion detected. Sending webhook..."); sendMotionEvent(); motionAlreadyReported = true; lastNoMotionTime = 0;
  }
  if (motion != MOTION_ACTIVE_STATE && motionAlreadyReported) {
...
void sendMotionEvent() {
  if (WiFi.status() != WL_CONNECTED) { logSensor("Wi-Fi offline. Motion event skipped."); return; }
  generateHardwareIds();
  WiFiClientSecure client; client.setInsecure();
  HTTPClient http; http.setTimeout(30000); http.setReuse(false);
  ...
  payload += "\"residentName\":\"" + jsonEscape(residentName) + "\",";
  payload += "\"message\":\"" + jsonEscape(message) + "\",";
  payload += "\"alertLevel\":\"Normal\",\"timeText\":\"ESP32 Motion Event\"";
  payload += "}";
  int responseCode = http.POST(payload);
  Serial.print(sensorLabel() + " | Webhook response code: "); Serial.println(responseCode);
  http.end(); client.stop(); delay(50);
}
...
void handleRoot() {
  ...
  html += "<div class='meta'><b>Setup ID:</b><br>" + htmlEscape(setupId) + "<br><br><b>Hardware ID:</b><br>" + htmlEscape(nodeId) + "<br><br><b>Firmware:</b><br>" + String(SOFTWARE_VERSION) + "<br><br><b>BLE:</b><br>Enabled<br><br><b>Auto firmware install:</b><br>Disabled</div>";
  html += "<form method='POST' action='/save'><label>Wi-Fi Network</label><select id='wifiName' name='wifiName'>";
  if (networkCount <= 0) html += "<option value=''>No Wi-Fi networks found</option>";
  else for (int i = 0; i < networkCount; i++) {
    String ssid = WiFi.SSID(i); int rssi = WiFi.RSSI(i); String selected = ssid == wifiName ? " selected" : "";
    html += "<option value='" + htmlEscape(ssid) + "'" + selected + ">" + htmlEscape(ssid) + " (" + String(rssi) + " dBm)</option>";
  }
  html += "</select><label>Wi-Fi Password</label><input id='wifiPassword' name='wifiPassword' type='password' value='" + htmlEscape(savedWifiPasswordFor(wifiName)) + "' required>";
  ...
  html += "<label>Device Name</label><input name='deviceName' value='" + htmlEscape(deviceName) + "' required>";
  html += "<button type='submit'>Activate Sensor</button></form>";
  html += "<p class='hint'><a href='/'>Refresh Wi-Fi List</a> &nbsp; | &nbsp; <a href='/reset'>Clear assignment</a></p><p class='hint'><a href='/factory-reset'>Factory reset saved Wi-Fi history</a></p>";
  html += "<script>var knownWifiPasswords=" + knownWifiMap + ";var wifiSelect=document.getElementById('wifiName');var passwordField=document.getElementById('wifiPassword');function autofillWifiPassword(){var ssid=wifiSelect.value;if(knownWifiPasswords[ssid]){passwordField.value=knownWifiPasswords[ssid];}}wifiSelect.addEventListener('change',autofillWifiPassword);autofillWifiPassword();</script>";
  html += "</div></body></html>";
  server.send(200, "text/html", html);
}

void handleSave() {
  wifiName = server.arg("wifiName"); wifiPassword = server.arg("wifiPassword");
  locationName = server.arg("locationName"); residentName = server.arg("residentName"); roomName = server.arg("roomName"); deviceName = server.arg("deviceName");
  saveSettings();
